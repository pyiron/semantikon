...
The `workflow` decorator from `flowrep.workflow` allows you to define a workflow that uses the above functions in semantikon. You can use any workflow management system that can export the workflow in the [`flowrep`](https://github.com/pyiron/flowrep)-format. Via `flowrep.workflow(semantikon.get_knowledge_graph)(my_kinetic_energy_workflow.flowrep_recipe)` you can extract a knowledge graph from the workflow. The knowledge graph schematically has the following structure:
