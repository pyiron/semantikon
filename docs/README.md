...
{'a': {'units': 'meter', 'uri': rdflib.term.URIRef('http://example.org/distance'), 'dtype': <


class 'float'>}, 'b': {'units': 'second', 'uri': rdflib.term.URIRef('http://example.org/time'), 'dtype': <


class 'float'>}}

>> > print(parse_output_args(get_speed))
{'units': 'meter/second', 'label': 'speed'

, 'uri': rdflib.term.URIRef('http://example.org/speed'), 'dtype': <
