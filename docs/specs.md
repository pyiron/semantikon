...
The following specs describe the main three steps for the semantic handling of workflows: Workflow representation (semantikon representation), translation to knowledge graphs and analysis.
...
- `pmd`: `https://w3id.org/pmd/co/PMD_`
- `ro`: `http://purl.obolibrary.org/obo/RO_`
- `sns`: `http://pyiron.org/ontology/` (semantikon namespace)
...
- **Process** (`bfo:0000015`): A workflow node, applied to both workflows and atomic nodes
- **Input assignment** (`pmd:0000066`): Input port of a node, i.e. input argument in the sense of python. This node only specifies which argument, but does not contain any metadata (such as value, datatype etc.)
- **Output assignment** (`pmd:0000066`): Analogous to input assignment
- **Value specification** (`obi:0001933`): Data node attached to an input and/or output assignment. This node contains all the metadata attached to the input and output ports, such as value, URI, ontological triples, restrictions etc.

Here are the corresponding predicates:

- **has_part** (`bfo:0000051`): **process** to **input** and **output assignment**
- **has_participant** (`ro:0000057`): **input** and **output assignment** to **value specification**
...
In addition, the order of the process is denoted by **precedes** (`bfo:0000063`).

### Semantic description of python functions

To be added

### Handling of URI, units and restrictions for input/output arguments

A URI (`ex:uri` in the example below) is treated differently depending on whether it was defined for the input or the output. If defined for the input argument, T-box gets the following triples:

```turtle
sns:data_node rdfs:subClassOf [ a owl:Restriction ;
            owl:allValuesFrom ex:uri ;
...
The unit follows the same pattern. If defined for the input:

```turtle
sns:data_node rdfs:subClassOf [ a owl:Restriction ;
            owl:hasValue unit:some_units ;
...
```turtle
:datanode qudt:hasUnit unit:some_units .
```
...
Restrictions can be defined only for input arguments. If defined for output arguments, they are ignored. They can be expressed as OWL restrictions or SHACL shapes. In both cases, RDF predicate–object pairs (i.e. the “doubles” mentioned above) are expected, where the subject node (blank node) is automatically generated inside `semantikon` and appended. The blank node is attached to the data node via `rdfs:subClassOf` in case of OWL, or `sh:targetClass` in case of SHACL.
...
### Translation of OWL restrictions to SHACL shapes
